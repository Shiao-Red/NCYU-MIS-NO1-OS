...
if(cluster.isMaster){//是否為主程序
	...
	*/
	io.on('connection', (socket)=>{
		...
		io.to(attendedRoom).emit('numberOfPersonChange',{
			number: allRooms.get(attendedRoom).length,
			userName: userName,
			status: 'join'
		});
		...
		socket.on('clientProfile', ()=>{ //client端的請求
			io.to(attendedRoom).emit('serverProfile',{
				isGuestOrHost: isGuestOrHost,
				attendedRoom: attendedRoom,
				userName: userName
			});
